...
export function MAQuestionImporter() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("import-ma-questions");

      if (error) throw error;

      if (data.alreadyExists) {
        toast.info("MA questions already imported");
        setStatus("success");
        setImportedCount(0);
      } else {
        toast.success(`Successfully imported ${data.count} MA questions!`);
        setStatus("success");
        setImportedCount(data.count);
      }
    } catch (error) {
      console.error("Import error:", error);
      toast.error("Failed to import MA questions. Please try again.");
      setStatus("error");
    } finally {
      setImporting(false);
    }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2 text-sm text-muted-foreground">
        ...
        <ul className="list-disc list-inside space-y-1">
          <li>MCQ questions for MA paper</li>
          <li>Questions organized by unit code (MA01-MA16)</li>
