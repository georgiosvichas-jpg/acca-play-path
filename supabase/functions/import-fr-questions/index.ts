...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting FR questions import...');
    ...
    const publicUrl = `${supabaseUrl}/storage/v1/object/public/data/fr_question_bank.json`;
    ...
    const questions: FRQuestion[] = await fileResponse.json();
    ...
    if (existingQuestions && existingQuestions.length > 0) {
      console.log('FR questions already exist in database');
      return new Response(
        JSON.stringify({ 
          message: 'FR questions already imported',
          count: 0,
          alreadyExists: true
        }),
    ...
    console.log(`Successfully imported ${totalInserted} FR questions`);

    return new Response(
      JSON.stringify({ 
        message: 'FR questions imported successfully',
        count: totalInserted
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
    ...
    console.error('Error in import-fr-questions function:', error);
