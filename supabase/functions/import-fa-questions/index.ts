...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting FA questions import...');
    ...
    const questions: FAQuestion[] = await fileResponse.json();
    ...
    if (existingQuestions && existingQuestions.length > 0) {
      console.log('FA questions already exist in database');
      return new Response(
        JSON.stringify({ 
          message: 'FA questions already imported',
          count: 0,
          alreadyExists: true
        }),
    ...
    console.log(`Successfully imported ${totalInserted} FA questions`);

    return new Response(
      JSON.stringify({ 
        message: 'FA questions imported successfully',
        count: totalInserted
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
    ...
    console.error('Error in import-fa-questions function:', error);
