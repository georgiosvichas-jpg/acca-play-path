...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (hookSecret) {
      ...
      const resetUrl = `${Deno.env.get('SUPABASE_URL')}/auth/v1/verify?token=${email_data.token_hash}&type=${email_data.email_action_type}&redirect_to=${email_data.redirect_to}`
                  ...
                  <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <tr>
                      <td style="padding: 40px 40px 30px 40px;">
                        ...
                        <p style="margin: 8px 0 0 0; font-size: 12px; line-height: 18px; color: #a0aec0; word-break: break-all;">${resetUrl}</p>
                        <p style="margin: 30px 0 0 0; font-size: 14px; line-height: 20px; color: #718096;">This link will expire in 60 minutes for security reasons.</p>
                      </td>
                    </tr>
                    <tr>
                      <td style="padding: 20px 40px; background-color: #f7fafc; border-top: 1px solid #e2e8f0; border-radius: 0 0 8px 8px;">
                        <p style="margin: 0; font-size: 12px; line-height: 18px; color: #718096;">If you didn't request a password reset, you can safely ignore this email. Your password will remain unchanged.</p>
                      </td>
                    </tr>
                  </table>
