...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch the FM question bank JSON from storage
    const jsonUrl = `${supabaseUrl}/storage/v1/object/public/data/fm_question_bank.json`;
    const response = await fetch(jsonUrl);
    
    if (!response.ok) {
      throw new Error(`Failed to fetch FM question bank: ${response.statusText}`);
    }

    const questions: FMQuestion[] = await response.json();
    
    if (!Array.isArray(questions)) {
      throw new Error('Invalid JSON format: expected an array of questions');
    }

    console.log(`Loaded ${questions.length} FM questions from JSON`);

    // Check if FM questions already exist
    ...
    if (existingCount && existingCount > 0) {
      return new Response(
        JSON.stringify({ 
          message: 'FM questions already imported',
          count: existingCount,
    ...
    return new Response(
      JSON.stringify({ 
        message: 'FM questions import complete',
        imported,
        errors,
